...
function filterTarget(target) {
  ...
  if (args.excludeTargets && args.excludeTargets.indexOf(target.name) >= 0) {
    console.log('excluding ' + target.name)
    include = false;
  }
  // added ability to use "*" wildcard in exclude target at end of target name
  if(args.excludeTargets && args.excludeTargets.toString().indexOf("*") >= 0) {
    var badTargets = args.excludeTargets.toString().split(',');
    // find the targets with wildcards
    for(var i = 0 ; i < badTargets.length ; i++) {
      var bt = badTargets[i];

      // only accept wildcards at the end of a target
      if(bt.indexOf('*') === (bt.length - 1) ) {

        // exclude targets that begin with current exclude target
        if(target.name.indexOf(bt.substring(0, bt.length - 1)) == 0) {
          include = false;
        }
      }
    }
